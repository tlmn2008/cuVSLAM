 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
/* CoreX add: the software texture fetch below is gated on __CUDA_ARCH__ (device
 * pass only). clang-cuda still parses __global__ kernel bodies during the HOST
 * pass, so tex2D<> must be *declared* there or the host pass fails with
 * "use of undeclared identifier 'tex2D'". Provide a host-visible forward
 * declaration matching the device definition; it is never odr-used on the host
 * (only referenced inside __global__ bodies), so no definition is needed. */
